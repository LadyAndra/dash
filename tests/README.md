...
| File | What it covers |
|---|---|
| `release-safety.test.mjs` | Publishing guardrail. Checks that every file named in `sw.js` exists, every active local asset loaded from `index.html`, `manifest.json`, CSS, and ES-module imports is covered by the offline `SHELL`, and — when git history is available — a commit that changes a cached app file also changes `sw.js`, while every `sw.js` change bumps `CACHE_VERSION`. Needs no package. |
